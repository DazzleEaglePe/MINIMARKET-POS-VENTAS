...
-- Control de acceso para superadmin
-- Tabla de emails permitidos para ser superadmin
DO $$ BEGIN
        IF NOT EXISTS (
                SELECT 1 FROM information_schema.tables 
                WHERE table_schema='public' AND table_name='allowed_superadmins'
        ) THEN
                CREATE TABLE public.allowed_superadmins (
...
-- Función segura para bootstrap del usuario tras login
-- Crea el usuario si no existe. Asigna rol superadmin solo si su email está en allowed_superadmins.
-- Crea empresa automáticamente si resultó superadmin y aún no existe.
...
BEGIN
    ...
    SELECT * INTO v_user FROM usuarios WHERE id_auth = p_id_auth LIMIT 1;
        ...
        SELECT id INTO v_role_super FROM roles WHERE lower(nombre) = 'superadmin' LIMIT 1;
            -- prioridad de rol por defecto: pendiente (cero permisos) > cajero > gerente > admin
    ...
    -- Si quedó como superadmin, asegurar empresa
    IF v_role_super IS NOT NULL AND v_user.id_rol = v_role_super THEN
        ...
        IF v_emp.id IS NULL THEN
            INSERT INTO empresa (id_auth, correo, id_usuario)
            VALUES (p_id_auth, p_email, v_user.id);
        END IF;
    END IF;

     -- Asignar permisos por defecto únicamente si no es superadmin NI pendiente
...
-- Asegurar existencia de roles base: superadmin, gerente, admin, cajero
DO $$ BEGIN
    ...
    END IF;
    -- Rol sin permisos para nuevos usuarios hasta aprobación manual
...
-- Asignar permisos por defecto a un usuario basado en su rol, evitando duplicados
...
BEGIN
    ...
    -- Insertar permisos definidos por rol, evitando duplicados
    FOR rec IN
        SELECT pd.id_modulo
...
-- Semillas idempotentes de permisos por defecto por rol
DO $$
DECLARE
...
BEGIN
    ...
    -- NOTA: no se generan permisos_dafault para 'pendiente' (cero permisos por diseño)

    -- módulos (por link)
    SELECT id INTO m_home FROM modulos WHERE link = '/' LIMIT 1;
    SELECT id INTO m_pos FROM modulos WHERE link = '/pos' LIMIT 1;
    SELECT id INTO m_cobrar FROM modulos WHERE nombre ILIKE 'cobrar venta' LIMIT 1; -- link '-'
    ...
    -- Cajero: Home, POS, Cobrar, Clientes
    IF r_cajero IS NOT NULL THEN
        IF m_home IS NOT NULL THEN INSERT INTO permisos_dafault(id_rol, id_modulo)
            SELECT r_cajero, m_home WHERE NOT EXISTS (SELECT 1 FROM permisos_dafault WHERE id_rol=r_cajero AND id_modulo=m_home);
        END IF;
        IF m_pos IS NOT NULL THEN INSERT INTO permisos_dafault(id_rol, id_modulo)
            SELECT r_cajero, m_pos WHERE NOT EXISTS (SELECT 1 FROM permisos_dafault WHERE id_rol=r_cajero AND id_modulo=m_pos);
        END IF;
        IF m_cobrar IS NOT NULL THEN INSERT INTO permisos_dafault(id_rol, id_modulo)
            SELECT r_cajero, m_cobrar WHERE NOT EXISTS (SELECT 1 FROM permisos_dafault WHERE id_rol=r_cajero AND id_modulo=m_cobrar);
        END IF;
        IF m_clientes IS NOT NULL THEN INSERT INTO permisos_dafault(id_rol, id_modulo)
            SELECT r_cajero, m_clientes WHERE NOT EXISTS (SELECT 1 FROM permisos_dafault WHERE id_rol=r_cajero AND id_modulo=m_clientes);
        END IF;
    END IF;

    -- Gerente: navegación principal, inventarios y configuración operacional moderada
    ...
    -- Admin: casi todo (gestión completa)
...
CREATE or replace FUNCTION public.buscarusuariosasignados(_id_empresa integer, buscador text) RETURNS TABLE(id_asignacion integer, id_usuario integer, usuario text, sucursal text, caja text, rol text, email text, estadouser text,id_rol integer,nro_doc text, telefono text)
    LANGUAGE sql
    AS $$
...
DECLARE
    ...
    rec RECORD;
BEGIN
    -- Solo aplica para roles distintos de 'superadmin' y 'pendiente'
    ...
    -- Permisos por defecto según rol (permisos_dafault), excluyendo 'pendiente'
        FOR rec IN
            SELECT pd.id_modulo
            FROM permisos_dafault pd
            WHERE pd.id_rol = v_id_rol AND pd.id_modulo IS NOT NULL
...
begin
 ...
 -- resolver roles: superadmin y admin
...
--usuario admin
 ...
 values(
     ...
     CASE
         ...
         ELSE COALESCE(id_admin_rol, id_superadmin_rol) -- fallback: si no hay rol admin, usa el que exista
     END,
     new.correo,
     new.id_auth
 )
...
--
 --insertar permisos solo si es superadmin: todos los módulos
...
CREATE or replace FUNCTION public.mostrarusuariosasignados(_id_empresa integer) RETURNS TABLE(id_asignacion integer, id_usuario integer, usuario text, sucursal text, caja text, rol text, email text, estadouser text,id_rol integer, nro_doc text, telefono text)
    LANGUAGE sql
    AS $$
select asignacion_sucursal.id as id_asignacion,
 usuarios.id as id_usuario,
...
INSERT INTO public.tipo_comprobantes VALUES (5, 'Ticket', 'ventas');
